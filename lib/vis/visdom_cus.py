...
class VisTracking(VisBase):
    ...
    def draw_data(self):
        ...
        resize_factor = 1
        if max(disp_image.shape) > 960:
            resize_factor = 960.0 / float(max(disp_image.shape))
        ...
        for i, disp_rect in enumerate(boxes):
            ...
            # NaN 检查
            if any(torch.isnan(disp_rect)):
                print(f"Skipping NaN box at index {i}: {disp_rect}")
                continue  # 如果包含 NaN，则跳过当前框
        ...
        # 如果需要控制显示大小，用 opts 指定
        # 转为 torch CHW，归一化到 [0,1]
        # disp_image = torch.from_numpy(disp_image.copy())  # HWC -> tensor HWC
        # if disp_image.dim() == 4:
        #     disp_image = disp_image.squeeze(0)
        # disp_image = disp_image.permute(2, 0, 1).float() / 255.0  # CHW, [0,1]
        # opts = {'title': self.title, 'width': disp_image.shape[2], 'height': disp_image.shape[1]}

        if len(self.raw_data) > 3:
            self.visdom.image(disp_image, opts={'title': self.title, 'width': disp_image.shape[2], 'height': disp_image.shape[1],**self.raw_data[3]}, win=self.title)
            # self.visdom.image(disp_image, opts=opts, win=self.title)
        else:
